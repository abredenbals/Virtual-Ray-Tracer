...
using _Project.Ray_Tracer.Scripts.Ray_Marching.RM_Camera;
...
namespace _Project.Ray_Tracer.Scripts.Ray_Marching
{
    ...
    /// to produce a relatively small number of rays for the <see cref="RayMarchingManager"/> to visualize. For larger images
    /// (and no ray trees) the <see cref="RenderImage"/> function should be used.
    /// </summary>
    public class UnityRayMarcher : MonoBehaviour
    {
        ...
        static private UnityRayMarcher instance = null;
        private RMSceneManager rmSceneManager;

        private RTScene scene;
        private new RMCamera camera;
        ...
        /// <summary>
        /// Get the current <see cref="UnityRayMarcher"/> instance.
        /// </summary>
        /// <returns> The current <see cref="UnityRayMarcher"/> instance. </returns>
        public static UnityRayMarcher Get()
        {
            return instance;
        }

        /// <summary>
        /// Render the current <see cref="RMSceneManager"/>'s <see cref="RTScene"/> while building up a list of ray trees.
        /// </summary>
        /// <returns> The list of ray trees that were traced to render the image. </returns>
        public List<TreeNode<RMRay>> Render()
        {
            List<TreeNode<RMRay>> rayTrees = new List<TreeNode<RMRay>>();
            scene = rmSceneManager.Scene;
            camera = scene.Camera;
            ...
            for (int y = 0; y < height; ++y)
            {
                for (int x = 0; x < width; ++x)
                {
                    ...
                    // for the length of the returned RMRay. Since we have this factor we also use it to normalize this
                    ...
                    TreeNode<RMRay> rayTree = Trace(origin + pixel,
                                                    pixel / pixelDistance, // Division by magnitude == .normalized.
                                                    MaxDepth, RMRay.RayType.Normal);
        ...
        private TreeNode<RMRay> Trace(Vector3 origin, Vector3 direction, int depth, RMRay.RayType type)
        {
            RaycastHit hit;
            bool intersected = Physics.Raycast(origin, direction, out hit, Mathf.Infinity, rayTracerLayer);
            
            TreeNode<RMRay> rayTree = new TreeNode<RMRay>(new RMRay());

            // If we did not hit anything we return a no hit ray whose result color is black.
            if (!intersected)
            {
                rayTree.Data = new RMRay(origin, direction, Mathf.Infinity, BackgroundColor, RMRay.RayType.NoHit);
                return rayTree;
            }
            ...
            rayMarch(origin, direction, out hitInfo, 25, 999.0f , rayTracerLayer);
            ...
            rayTree.Data = new RMRay(origin, direction, hit.distance, ClampColor(color), type);
            return rayTree;
        }

        private RMRay TraceLight(ref Vector3 lightVector, RTLight light, in HitInfo hitInfo)
        {
            ...
            if (RenderShadows)
            {
                ...
                if (Physics.Raycast(shadowOrigin, lightVector, out shadowHit, lightDistance, rayTracerLayer))
                    return new RMRay(hitInfo.Point, lightVector, shadowHit.distance, Color.black,
                        RMRay.RayType.Shadow);
            }
            ...
            return new RMRay(hitInfo.Point, lightVector, lightDistance, ClampColor(color), RMRay.RayType.Light);
        }

        private List<TreeNode<RMRay>> TraceReflectionAndRefraction(int depth, in HitInfo hitInfo)
        {
            List<TreeNode<RMRay>> rays = new List<TreeNode<RMRay>>();
            TreeNode<RMRay> node;
        ...
        /// <summary>
        /// Render the current <see cref="RMSceneManager"/>'s <see cref="RTScene"/> while building up a "high resolution"
        /// image.
        /// </summary>
        ...
        private bool rayMarch(Vector3 origin, Vector3 direction, out HitInfo hit, int maxIterations,
            float maxDistance, int layerMask)
        {
            ...
            for (int iteration = 0; iteration < maxIterations; iteration++)
            {
                ...
                {
                    float distance = mesh.DistanceToPoint(origin);
                    if (distance < minDist)
                    {
                        minDist = distance;
                        nearestObject = mesh;
                ...
                if (minDist < epsilon)
                {
                    Vector3 normalPlaceholder = Vector3.one;
